...
/// Base model class macro extension definitions.
///
macro class GsarModelMacro implements ClassDeclarationsMacro {
  /// Default, unnamed class constructor.
  ///
  const GsarModelMacro();
  ...
  Future<void> buildDeclarationsForClass(
    ...
    for (final field in fields) {
      ...
      final fieldType = (field.type.code.asNonNullable as NamedTypeAnnotationCode).name.name;
      fieldTypes[fieldName] = fieldType;
      if (_collectionTypes.contains(fieldType)) {
        final generics = (field.type.code.asNonNullable as NamedTypeAnnotationCode)
            .typeArguments
            .map((e) => (e.asNonNullable as NamedTypeAnnotationCode).name.name)
            .toList();
    ...
    final fieldsMapper = fields.map(
      (field) {   
        return (
          key: '\'${field.identifier.name}\'',
          type: (field.type.code.asNonNullable as NamedTypeAnnotationCode).name.name,
          isRequired: !field.type.isNullable,
        );
      },
    ).toList();
    builder.declareInType(
      DeclarationCode.fromParts(
        [
          'List<({String key, Type type, bool isRequired})> generateFields() {return ',
          '$fieldsMapper',
          ';}',
        ],
      ),
