...
class GsaaApiUsers extends GsarApi {
  ...
  Future<String> register({
    ...
    final response = await post(
      ...
      GsaaModelUser(
          ...
          contact: GsaaModelContact(email: email, phoneCountryCode: phoneCountryCode, phoneNumber: phoneNumber),
  ...
  Future<String> login({required String username, required String password}) async {
    final response = await post(GsaaEndpointsUsers.login.path, {'username': username, 'password': password});
  ...
  Future<String> loginPassword({required String username, required String password}) async {
    final response = await post(GsaaEndpointsUsers.loginPassword.path, {'password': password});
    final securityToken = response['securityToken'];
    if (securityToken is! String) {
      throw 'Security token is missing.';
    }
  ...
  Future<GsaaModelUser> getUserDetails([String? userId]) async {
    final response = await get(GsaaEndpointsUsers.getUserDetails.path + (userId != null ? '?userId=$userId' : ''));
    try {
  ...
  Future<void> editUserDetails({String? username}) async {
    await patch(GsaaEndpointsUsers.editUserDetails.path, {if (username != null) 'username': username});
  }
  ...
  Future<void> requestDeletion({required String email, required String password}) async {
    await delete(GsaaEndpointsUsers.requestDeletion.path, body: {'email': email, 'password': password});
  }

  /// Deletes the user data from the database.
  ///
  Future<void> deleteAll({required String password}) async {
    await delete(GsaaEndpointsUsers.deleteAll.path, body: {'password': password});
  }
  ...
  Future<void> deleteSoft({required String password}) async {
    await delete(GsaaEndpointsUsers.deleteSoft.path, body: {'password': password});
  }
}
